...
class DatasetCOCO(Dataset):
    ...
    def build_class_ids(self):
        if self.fold == 0:
            class_ids_val = [0, 2, 3, 6]
        elif self.fold == 1:
            class_ids_val = [1, 7, 11, 13]
        elif self.fold == 2:
            class_ids_val = [4, 5, 8, 9]
        elif self.fold == 3:
            class_ids_val = [10, 12, 14, 15]
        class_ids_trn = [x for x in range(self.nclass) if x not in class_ids_val]
        class_ids = class_ids_trn if self.split == 'trn' else class_ids_val
        return class_ids

    def build_img_metadata_classwise(self):
        with open('/home/ljz/fewshot_HS_main/data/splits/coco/%s/fold%d.pkl' % (self.split, self.fold), 'rb') as f:
        #with open('/home/ljz/lwh/fewshot_HSN_main/data/splits/coco/%s/fold%d.pkl' % (self.split, self.fold), 'rb') as f:
